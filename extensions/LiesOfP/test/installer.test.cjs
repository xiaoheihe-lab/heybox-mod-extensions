...
verifyAdapter(require('../dist/LiesOfP/index.cjs'), {"id":1627720,"folder":"LiesofP","suffix":false,"executable":"LOP.exe"})

const { default: main, testRoot, installRoot, testPak, installPak } = require('../dist/LiesOfP/index.cjs')

test('root installer preserves all supported game subtrees and filenames', () => {
  for (const wrapper of ['', 'Download/Mod/']) {
    const paths = ['LiesofP/Content/Paks/a.pak', 'LiesofP/Content/Paks/~mods/A/b.pak', 'LiesofP/Content/Paks/~mods/B/c.pak', 'LiesofP/Binaries/Win64/loader.dll', 'LiesofP/Config/DefaultEngine.ini']
    const files = [...paths.map(p => wrapper + p), wrapper + 'readme.txt', wrapper + 'stray.pak', wrapper + 'Other/game.dll']
    assert.equal(testRoot(files, '1627720').supported, true)
    assert.equal(testPak(files, 1627720).supported, false)
    assert.deepEqual(installRoot(files), { modType: '1627720-root', instructions: paths.map(p => ({ type: 'copy', source: wrapper + p, destination: p })) })
  }
})

test('root tester rejects unrelated games, bare archives, FOMOD and unsafe paths', () => {
  assert.equal(testRoot(['LiesofP/Content/Paks/a.pak'], 1).supported, false)
  for (const files of [[], ['a.pak'], ['Paks/a.pak'], ['LiesofP/Unknown/a.txt'], ['../LiesofP/Content/a.pak'], ['LiesofP/Content/../a.pak'], ['LiesofP/Content/a.pak/'], ['LiesofP/Content/a.pak', 'Wrap/FOMOD/ModuleConfig.xml']]) {
    assert.equal(testRoot(files, 1627720).supported, false)
    assert.throws(() => installRoot(files), /no supported root layout/)
  }
  assert.equal(testPak(['Wrap/a.pak'], 1627720).supported, true)
})
...
test('root installer keeps original sources, rejects collisions and ambiguous variants', () => {
  ...
  assert.deepEqual(installRoot([source, source]).instructions, [{type:'copy', source, destination:'LiesofP/Content/Paks/a.pak'}])
  assert.throws(() => installRoot(['LiesofP/Content/a.pak', 'liesofp/content/A.PAK']), /multiple archive files target/)
  assert.throws(() => installRoot(['A/LiesofP/Content/a.pak', 'B/LiesofP/Content/b.pak']), /multiple game roots/)
})
...
test('root registration wins over loose Pak and resolves destinations against game root', () => {
  ...
  const root = types.find(row => row[0] === '1627720-root')
  assert.equal(root[1], 30)
  assert.equal(root[2](1627720), true)
  assert.equal(root[2](1), false)
  assert.equal(root[3](), '{gamePath}')
  assert.equal(root[4](undefined), false)
  const files = ['Wrap/LiesofP/Content/Paks/~mods/A/a.pak', 'Wrap/LiesofP/Content/Paks/~mods/B/b.pak', 'loose.pak']
  assert.equal(root[4]({files}), true)
  const chosen = installers.sort((a,b) => b[1]-a[1]).find(row => row[2](files, 1627720).supported)
  assert.deepEqual(chosen, ['1627720-root', 30, testRoot, installRoot])
  assert.equal(root[3]() + '/' + chosen[3](files).instructions[1].destination, '{gamePath}/LiesofP/Content/Paks/~mods/B/b.pak')
  assert.equal(installers.find(row => row[2](['a.pak'], 1627720).supported)[3], installPak)
})
