...
function selectPaks(files) {
  const entries2 = entriesFor(files);
  if (entries2.some(({ file }) => /(^|\/)fomod\/moduleconfig\.xml$/i.test(file) || /(^|\/)logicmods\//i.test(file) || /\.(dll|asi|lua)$/i.test(file))) return [];
  const first = entries2.find(({ file }) => /\.pak$/i.test(file));
  if (!first) return [];
  const root = first.file.slice(0, first.file.lastIndexOf("/") + 1);
  return entries2.filter(({ file }) => file.toLowerCase().startsWith(root.toLowerCase()) && /\.pak$/i.test(file)).map((entry) => ({ ...entry, destination: entry.file.slice(root.length) }));
}
...
function installPakArchive(files, options) {
  ...
  for (const pak of paks) {
    ...
    const companions = entries2.filter(({ file }) => /\.(sig|ucas|utoc)$/i.test(file) && file.slice(0, file.lastIndexOf(".")).toLowerCase() === stem);
...
function rootFiles(files) {
  return files.flatMap((source) => {
    ...
    const anchor = parts.findIndex((part) => part.toLowerCase() === "projectwingman");
    if (anchor < 0 || parts.length < anchor + 3) return [];
    if (!["content", "binaries", "config"].includes(parts[anchor + 1].toLowerCase())) return [];
    return [{ source, prefix: parts.slice(0, anchor).join("/"), destination: ["ProjectWingman", ...parts.slice(anchor + 1)].join("/") }];
  });
}
...
function installRoot(files) {
  if (!testRoot(files, GAME_ID).supported) throw new Error("Project Wingman: no supported root layout (FOMOD requires a separate installer)");
  const entries2 = rootFiles(files);
  if (new Set(entries2.map((entry) => entry.prefix.toLowerCase())).size !== 1) {
    throw new Error("Project Wingman: multiple game roots; select one mod variant before installing");
  }
  const destinations = /* @__PURE__ */ new Map();
  const instructions = [];
  for (const { source, destination } of entries2) {
    const key = destination.toLowerCase();
    const previous = destinations.get(key);
    if (previous === source) continue;
    if (previous !== void 0) throw new Error(`Project Wingman: multiple archive files target ${destination}`);
    destinations.set(key, source);
    instructions.push({ type: "copy", source, destination });
  }
...
// skin-installer.ts
var GAME_ID2 = 895870;
var SKIN_MOD_TYPE = `${GAME_ID2}-skin`;
...
function testSkin(files, gameId) {
  return { supported: String(gameId) === String(GAME_ID2) && isSkinArchive(files), requiredFiles: [] };
}
function destinationParts(list) {
...
function installSkin(files) {
  if (!testSkin(files, GAME_ID2).supported) throw new Error("Project Wingman: no supported skin archive");
  const list = entries(files);
...
// index.ts
var GAME_ID3 = 895870;
var MOD_TYPE = `${GAME_ID3}-pak`;
var MOD_PATH = "ProjectWingman/Content/Paks/~mods";
var PRIORITY = 25;
var OPTIONS = { gameId: GAME_ID3, gameName: "Project Wingman", modType: MOD_TYPE, requirePatchSuffix: false };
...
function main(context) {
  context.registerGame({
    ...
    queryPath: async () => (await context.api.util.GameStoreHelper.findByAppId(String(GAME_ID3)))?.gamePath,
    requiredFiles: ["ProjectWingman.exe"],
    environment: { SteamAPPId: String(GAME_ID3) },
    details: { steamAppId: GAME_ID3, nexusGameDomainName: "projectwingman", customOpenModsPath: MOD_PATH }
  });
  context.registerModType(
    MOD_TYPE,
    PRIORITY,
    (gameId) => String(gameId) === String(GAME_ID3),
    () => "{gamePath}/" + MOD_PATH,
    (input) => {
      const files = Array.isArray(input) ? input : input?.files;
      return Array.isArray(files) && testPak(files.map(String), GAME_ID3).supported;
    },
    { name: "Project Wingman Pak Mod" }
  );
  ...
  context.registerModType(
    ...
    (input) => {
      const files = Array.isArray(input) ? input : input?.files;
      return Array.isArray(files) && testRoot(files.map(String), GAME_ID3).supported;
    },
    { name: "Project Wingman Root Mod" }
  );
  ...
  context.registerModType(
    ...
    28,
    (gameId) => String(gameId) === String(GAME_ID3),
    () => "{gamePath}/ProjectWingman/Mods/Skins",
    (input) => {
      const files = Array.isArray(input) ? input : input?.files;
      return Array.isArray(files) && testSkin(files.map(String), GAME_ID3).supported;
    },
    { name: "Project Wingman Skin Mod" }
  );
  context.registerInstaller(SKIN_MOD_TYPE, 28, testSkin, installSkin);
  return true;
}
