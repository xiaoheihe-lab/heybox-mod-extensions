...
function rootFiles(files) {
  return files.flatMap((source) => {
    ...
    const anchor = parts.findIndex((part) => part.toLowerCase() === "codevein");
    if (anchor < 0 || parts.length < anchor + 3) return [];
    if (!["content", "binaries", "config"].includes(parts[anchor + 1].toLowerCase())) return [];
    return [{ source, prefix: parts.slice(0, anchor).join("/"), destination: ["CodeVein", ...parts.slice(anchor + 1)].join("/") }];
  });
}
...
function installRoot(files) {
  if (!testRoot(files, GAME_ID).supported) throw new Error("Code Vein: no supported root layout (FOMOD requires a separate installer)");
  const entries = rootFiles(files);
  if (new Set(entries.map((entry) => entry.prefix.toLowerCase())).size !== 1) {
    throw new Error("Code Vein: multiple game roots; select one mod variant before installing");
  }
  const destinations = /* @__PURE__ */ new Map();
  const instructions = [];
  for (const { source, destination } of entries) {
    ...
    if (previous !== void 0) throw new Error(`Code Vein: multiple archive files target ${destination}`);
    destinations.set(key, source);
    instructions.push({ type: "copy", source, destination });
  }
...
var MOD_PATH = "CodeVein/Content/Paks/~mods";
var PRIORITY = 25;
var OPTIONS = { gameId: GAME_ID2, gameName: "Code Vein", modType: MOD_TYPE, requirePatchSuffix: false };
...
function main(context) {
  context.registerGame({
    ...
    requiredFiles: ["CodeVein/Binaries/Win64/CodeVein-Win64-Shipping.exe"],
    environment: { SteamAPPId: String(GAME_ID2) },
    details: { steamAppId: GAME_ID2, nexusGameDomainName: "codevein", customOpenModsPath: MOD_PATH }
  });
