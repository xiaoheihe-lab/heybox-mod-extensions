...
function normalizeReEnginePakFiles(pathApi, mutation, options = {}) {
  ...
  for (const entry of mutation.entries) {
    ...
    if (entry.exists === false && mutation.adoptDeployment) {
      const candidates = (mutation.gameFiles || []).filter((file) => archiveExtName(file.targetPath) === ".pak" && file.hash === entry.expectedHash && !file.managed);
...
// index.ts
var GAME_ID = 2050650;
var GAME_NAME = "Resident Evil 4";
var EXECUTABLE = "re4.exe";
var REFRAMEWORK_DLL = "dinput8.dll";
var REFRAMEWORK_MOD_ID = 963;
...
async function getGameRoot(context) {
  ...
  if (!gamePath) {
    throw new Error(`Resident Evil 4 game path is unavailable: appid=${GAME_ID}`);
  }
  return gamePath;
}
function getReframeworkRequirements() {
  return [
    {
      key: "resident-evil-4-reframework",
      name: "REFramework",
...
function registerReframeworkLoader(context) {
  ...
  context.registerInstaller(
    ...
    (files) => ({
      ...installReEngineReframeworkLoader(context.api.util.path, files),
...
function registerReframework(context) {
  ...
  context.registerInstaller(
    ...
    (files) => ({
      ...installReEngineReframework(context.api.util.path, files),
...
function registerAutorun(context) {
  ...
  context.registerInstaller(
    ...
    (files) => ({
      ...installReEngineAutorun(context.api.util.path, files),
...
function registerPlugins(context) {
  ...
  context.registerInstaller(
    ...
    (files) => ({
      ...installReEnginePlugins(context.api.util.path, files),
...
function registerNatives(context) {
  ...
  context.registerInstaller(
    ...
    (files) => ({
      ...installReEngineNatives(context.api.util.path, files),
...
function registerPak(context) {
  ...
  context.registerInstaller(
    ...
    async (files) => {
      ...
      return {
        ...await installReEnginePak(context.api.util.path, context.api.util.fs, files, gameRoot),
...
async function main(context) {
  context.registerGame({
    ...
    details: {
      steamAppId: GAME_ID,
      nexusGameDomainName: "residentevil42023"
    }
  });
