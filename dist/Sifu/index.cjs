...
function installRoot(files) {
  ...
  for (const { source, destination } of entries) {
    ...
    if (previous !== void 0) throw new Error(`Sifu: multiple archive files target ${destination}`);
    destinations.set(key, source);
    instructions.push({ type: "copy", source, destination });
  }
...
// index.ts
var GAME_ID2 = 2138710;
var MOD_TYPE = `${GAME_ID2}-pak`;
var MOD_PATH = "Sifu/Content/Paks/~mods";
var PRIORITY = 25;
var OPTIONS = { gameId: GAME_ID2, gameName: "Sifu", modType: MOD_TYPE, requirePatchSuffix: false };
...
function main(context) {
  context.registerGame({
    ...
    requiredFiles: ["Sifu.exe"],
    environment: { SteamAPPId: String(GAME_ID2) },
    details: { steamAppId: GAME_ID2, nexusGameDomainName: "sifu", customOpenModsPath: MOD_PATH }
  });
