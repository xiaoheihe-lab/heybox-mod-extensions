...
import type { FomodApi, FomodDeploymentOptions } from '../protocol/types.js'
...
import { parseInfoXml, type FomodInfoMetadata } from '../parser/info.js'
import { asFomodError, FomodError } from './errors.js'
import { runFomod } from './run.js'

interface FomodCapableContext extends IExtensionContext {
  api: IExtensionContext['api'] & { util: IExtensionContext['api']['util'] & { fomod: FomodApi } }
}
...
async function sha256(value: string): Promise<string> {
  ...
  const cryptoApi = (globalThis as any).crypto?.subtle
  if (cryptoApi) {
...
export function registerFomodInstaller(contextValue: IExtensionContext, options: RegisterFomodInstallerOptions): void {
  const context = contextValue as FomodCapableContext
  const isTargetGame = (gameId: number | string) => String(gameId) === String(options.gameId)
  ...
  context.registerInstaller(options.typeId, options.priority ?? 100, (files, gameId) => ({
  ...
  }), async (files, stagingPath = '', deploymentOptions: any = {}) => {
    ...
    try {
      ...
      let info: FomodInfoMetadata | undefined
      ...
      const fomodOptions = (deploymentOptions?.fomod || {}) as FomodDeploymentOptions
