import type { FomodModel, FileItem, Option, Step } from '../model/types.js'
import type { FomodApi, FomodStoredState, FomodUiGroup } from '../protocol/types.js'
import { evaluateDependency, resolveOptionType, type EvaluationState } from '../evaluator/dependencies.js'
import { defaultSelections, validateStepSelections } from '../evaluator/selection.js'
import { expandFileItems, type CopyInstruction } from './files.js'
...
function flattenSelections(selections: Record<string, string[]>): Set<string> {
...
function replayState(
  ...
  files: EvaluationState['files'],
  throughStepIndex = model.steps.length - 1,
): EvaluationState {
  const state: EvaluationState = { flags: {}, files }
...
function optionTypes(step: Step, state: EvaluationState): Record<string, ReturnType<typeof resolveOptionType>> {
...
function isStoredSelectionValid(model: FomodModel, stored: FomodStoredState, files: EvaluationState['files']): boolean {
  try {
    const state: EvaluationState = { flags: {}, files }
...
async function buildUiGroups(
  step: Step,
  selectedIds: Set<string>,
  types: Record<string, ReturnType<typeof resolveOptionType>>,
  options: RunOptions,
): Promise<FomodUiGroup[]> {
...
function selectedOptions(model: FomodModel, selections: Record<string, string[]>): Option[] {
  const selected = flattenSelections(selections)
  return model.steps.flatMap((step) => step.groups.flatMap((group) => group.options.filter((option) => selected.has(option.id))))
}

function collectInstallFiles(model: FomodModel, selections: Record<string, string[]>, state: EvaluationState): FileItem[] {
...
export async function runFomod(options: RunOptions): Promise<{ instructions: Array<CopyInstruction | { type: 'attribute'; key: 'fomod'; value: FomodStoredState }>; state: FomodStoredState }> {
  ...
  if (!canReuse || options.forceInteractive) {
    ...
    try {
      while (cursor < options.model.steps.length) {
        ...
        if (!evaluateDependency(options.model.moduleDependencies, state)) throw new FomodError('FOMOD_INVALID_CONFIG', 'FOMOD module dependencies are not satisfied')
        ...
        const response = await options.api.requestStep({
          ...
          canGoBack: options.model.steps.slice(0, cursor).some((candidate, index) =>
            evaluateDependency(candidate.visible, replayState(options.model, selections, files, index - 1))
          ),
      ...
      await options.api.closeSession({ sessionId, status: (error as any)?.code === 'FOMOD_INSTALL_CANCELLED' ? 'cancelled' : 'failed', message: String((error as any)?.message || error) })
      throw error
    }
  }
  ...
  const instructions = [
    ...
    { type: 'attribute' as const, key: 'fomod' as const, value: state },
  ]
