...
export function normalizeArchivePath(input: unknown, allowEmpty = false): string {
...
export function joinArchivePath(...parts: unknown[]): string {
  return normalizeArchivePath(parts.filter((part) => String(part ?? '').trim()).join('/'), true)
}

export function findFomodRoot(files: string[]): { configPath: string; root: string } | null {
  const matches = files
