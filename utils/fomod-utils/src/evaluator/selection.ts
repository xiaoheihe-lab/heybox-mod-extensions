...
export function defaultSelections(group: Group, optionTypes: Record<string, FomodOptionType>): string[] {
...
export function validateStepSelections(
  step: Step,
  selectedIds: string[],
  optionTypes: Record<string, FomodOptionType>,
  ...
  for (const group of step.groups) {
    ...
    for (const option of group.options) {
      if (optionTypes[option.id] === 'Required' && !valid.includes(option.id)) valid.push(option.id)
    }
